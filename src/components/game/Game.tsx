...
import { selectUrl, selectName } from "../../features/image/image-slice";
import OptionBox from "./OptionBox";
import { useSelector, useDispatch } from "react-redux";
import AddScore from "./AddScore";
import CharTracker from "./CharTracker";
import { useStartGameMutation } from "../../features/game-api/game-api-slice";
...
export default function Game() {
  ...
  if (gameInfo) {
    return (
      <StyledMain>
        ...
        <StyledTop>
          ...
          {wrongAnswer && (
            <StyledWrongAnswer>Incorrect Coordinates!</StyledWrongAnswer>
          )}
        </StyledTop>
        <div>
          {imageURL !== "0" && imageName !== "0" && (
            <StyledImage
              data-testid="main-game-image"
              src={imageURL}
              alt={imageName}
              onClick={(e: ClickType) => {
                handleOnClick(e);
          ...
          {showBox ? (
            <OptionBox
              showWrong={showWrongAnswer}
              closeBox={closeShowBox}
              coordsProp={{
                top: coords.coordY,
                left: coords.coordX,
                adjustedX: coords.adjustedX,
                adjustedY: coords.adjustedY,
              }}
            >
              {" "}
            </OptionBox>
    ...
    return (
      <StyledFullErrorPage>
        Please start a game <Link to="/">Here</Link>{" "}
      </StyledFullErrorPage>
    );
  }
}
