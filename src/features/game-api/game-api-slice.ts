...
export const apiSlice = createApi({
  ...
  baseQuery: fetchBaseQuery({
    baseUrl: `https://${process.env.API_DOMAIN}` || "http://localhost:3000",
  }),
  ...
  endpoints: (builder) => ({
    ...
    updateGame: builder.mutation<
      ReturnMessage,
      { body: GameUpdateBody; gameid: string }
    >({
      query: ({ body, gameid }) => ({
        url: `/game/${gameid}`,
        method: "PUT",
      ...
      async onQueryStarted({ body, gameid }, lifecycleApi) {
        try {
          ...
          if (
            res.message === "Correct Coordinates" ||
            res.message === "Game Finished"
          ) {
            ...
            const patchResult = lifecycleApi.dispatch(
              apiSlice.util.updateQueryData(
                ...
                gameid,
                (draft) => {
                  const foundChar = draft.chars.find(
                    (char) => char.id === body.char,
                  );
                  if (foundChar) {
